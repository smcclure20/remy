				     ...
				     const bool trace, const double carefulness) const
{
  ...
  // run_actions.reset_counts();
  // std::vector< future< Evaluator::Outcome >> outcomes;

  // std::vector< NetConfig > run_configs;
  // run_configs = sample ? _sampled_configs : _configs;

  // for ( const auto & config : run_configs ) {
  //   outcomes.emplace_back(async(launch::async, [] ( T & run_actions,
  //                                         const unsigned int prng_seed,
  //                                         const NetConfig & config,
  //                                         const bool trace,
  //                                         const unsigned int ticks_to_run) { 
  //                                         return score_config(run_actions, prng_seed, config, trace, ticks_to_run); },
  //                                         run_actions, _prng_seed, config, trace, _tick_count * carefulness ));
  //   // auto test = std::async( std::launch::async, 
  //   // score_config, run_actions, (unsigned int)_prng_seed, config, (bool) trace, (unsigned int) (_tick_count * carefulness) );
  //   // outcomes.emplace_back(std::async( std::launch::async, 
  //   // score_config, run_actions, _prng_seed, config, trace, _tick_count * carefulness ));
  // }
  ...
  // Evaluator::Outcome total_outcome;
  // for (auto outcome : outcomes ){
  //     total_outcome.score += outcome.score;
  //     total_outcome.throughputs_delays.emplace_back(outcome.throughputs_delays);
  //     printf("Total whisker queries: %d", outcome.used_actions.total_queries());
  //     if ( total_outcome.used_actions.total_queries() < outcome.used_actions.total_queries() ) {
  ...
  // // Question: will parallelizing this cause issues in the whisker usage counts?
